    ...
    if (req.query.id) {
        ...
        connection.query(sql, function (err, result) {
            if (err) {
                ...
                if (result.length > 0) {
                    if (result[0].viewer_id != tk.obj.id && result[0].viewee_id != tk.obj.id) {
                        res.status(403).json({
                            data: '没有权限获取面试信息',
                        });
                    } else {
...
router.post('/', function (req, res, next) {
    if (req.body.viewee_id && req.body.start_time && req.body.finish_time) {
        var sql = `INSERT INTO interview (viewer_id,viewee_id,start_time,finish_time,status) VALUES (${tk.obj.id},${req.body.viewee_id},'${req.body.start_time}','${req.body.finish_time}','created')`;
        connection.query(sql, function (err, result) {
            if (err) {
